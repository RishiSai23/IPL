...
const Players = () => {
  ...
  const filteredPlayers = mockPlayers
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "age") { aValue = a.age; bValue = b.age; }
      if (sortBy === "auctionValue") { aValue = a.auctionValue.predicted; bValue = b.auctionValue.predicted; }
      if (sortBy === "runs") { aValue = a.stats.runs; bValue = b.stats.runs; }
      if (sortBy === "wickets") { aValue = a.stats.wickets; bValue = b.stats.wickets; }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-black">
      ...
      <div className="relative text-center py-14 bg-gradient-to-r from-purple-600 via-pink-500 to-orange-400 text-white shadow-lg rounded-b-3xl">
        <h1 className="text-5xl font-extrabold drop-shadow-lg">🏏 IPL Player Database</h1>
        <p className="mt-3 text-lg opacity-90">Stats, insights & auction predictions in one place</p>
        <div className="flex justify-center gap-6 mt-8">
          ...
          <Card className="bg-white/10 backdrop-blur-xl px-6 py-4 rounded-xl text-white shadow-lg">
            <h3 className="text-xl font-bold">{mockPlayers.reduce((sum,p)=>sum+(p.stats?.runs||0),0)}</h3>
            <p className="text-sm opacity-80">Total Runs</p>
          </Card>
          <Card className="bg-white/10 backdrop-blur-xl px-6 py-4 rounded-xl text-white shadow-lg">
            <h3 className="text-xl font-bold">{mockPlayers.reduce((sum,p)=>sum+(p.stats?.wickets||0),0)}</h3>
